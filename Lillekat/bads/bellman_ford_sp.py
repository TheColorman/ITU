...
class BellmanFordSP:
    ...
    def __init__(self, G: GenericDiGraph, s: int) -> None:
        self._distTo: list[float] = [
            sys.float_info.max
        ] * G.V()  # distTo[v] = distance  of shortest s->v path
        self._edgeTo: list[DirectedEdge | None] = [
            None
        ] * G.V()  # edgeTo[v] = last edge on shortest s->v path
        self._onQueue: list[bool] = [
            False
        ] * G.V()  # onQueue[v] = is v currently on the queue?
        self._queue: Queue[int] = Queue()  # queue of vertices to relax
        self._cost: int = 0  # number of calls to relax()
        self._cycle: Stack[DirectedEdge] | None = (
            None  # negative cycle (or None if no such cycle)
        )
    ...
    def _relax(self, G: GenericDiGraph, v: int) -> None:
        for e in G.adj(v):
            w = e.to_vertex()
    ...
    def negative_cycle(self) -> Stack[DirectedEdge] | None:
        return self._cycle
    ...
    def _find_negative_cycle(self) -> None:
        ...
        for v in range(V):
            if self._edgeTo[v] is not None:
                spt.add_edge(cast(DirectedEdge, self._edgeTo[v]))
    ...
    #         from the source vertex s
    # @throws IllegalArgumentException unless 0 <= v < V
    def path_to(self, v: int) -> Stack[DirectedEdge] | None:
        self._validate_vertex(v)
        if self.has_negative_cycle():
            raise UnsupportedOperationException("Negative cost cycle exists")
        if not self.has_path_to(v):
            return None
        path = Stack[DirectedEdge]()
        e = self._edgeTo[v]
        while e is not None:
            path.push(e)
    ...
    def _check(self, G: GenericDiGraph, s: int) -> bool:
        # has a negative cycle
        if self.has_negative_cycle():
            weight = 0.0
            for e in cast(Stack[DirectedEdge], self.negative_cycle()):
                weight += e.weight()
            if weight >= 0.0:
        ...
        # no negative cycle reachable from source
            ...
            for w in range(G.V()):
                if self._edgeTo[w] is None:
                    continue
                e = self._edgeTo[w]
                v = cast(DirectedEdge, e).from_vertex()
                if w != cast(DirectedEdge, e).to_vertex():
                    return False
                if self._distTo[v] + cast(DirectedEdge, e).weight() != self._distTo[w]:
