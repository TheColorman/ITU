...
class DijkstraUndirectedSP:
    ...
    def __init__(self, G: GenericGraph, s: int) -> None:
        ...
        self._dist_to: list[float] = [float("inf")] * G.V()
        self._edge_to: list[Edge | None] = [None] * G.V()
        self._dist_to[s] = 0.0
        self._validate_vertex(s)
        self._pq: IndexMinPQ = IndexMinPQ(G.V())
        self._pq.insert(s, 0)
    ...
    def path_to(self, v: int):
        ...
        e = self._edge_to[v]
        while e is not None:
            edge = cast(Edge, self._edge_to[x])
            path.push(edge)
            x = e.other(x)
            e = self._edge_to[x]
        return path
    ...
    def _relax(self, e: Edge, v: int) -> None:
