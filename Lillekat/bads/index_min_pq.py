...
from .errors import IllegalArgumentException, NoSuchElementException
...
class IndexMinPQ:
    ...
    def __init__(self, max_n: int) -> None:
        ...
        self.n: int = 0
        self.keys: list[float | None] = [None] * (max_n + 1)
        self.pq: list[int] = [0] * (max_n + 1)
        self.qp: list[int] = [-1] * (max_n + 1)
    ...
    def min_key(self) -> float:
        ...
        if self.n == 0:
            raise NoSuchElementException("Priority queue underflow")
        return cast(float, self.keys[self.pq[1]])
    ...
    def _greater(self, i: int, j: int) -> bool:
        ...
        """
        return cast(int, self.keys[self.pq[i]]) > cast(int, self.keys[self.pq[j]])
    ...
    def __iter__(self) -> Iterable[int]:
        ...
        for i in range(1, self.n + 1):
            copy.insert(self.pq[i], cast(int, self.keys[self.pq[i]]))
